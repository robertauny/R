...
## File:      nn.R
##
## Purpose:   A neural network for maximizing cost savings.
##
## Parameter: N/A
##
...
drv <- JDBC('com.mysql.jdbc.Driver'
           ,'/opt/mysql/mysql-connector-java-5.1.35-bin.jar'
           ,identifier.quote="'")
...
conn <- dbConnect(drv
                 ,'jdbc:mysql://localhost:3306/INTUIT'
                 ,user
                 ,pass)

if( dbg == 1 ) print("QUERY")

# SQL to get the data to be used in the model of counts of screen users
query <- paste(" SELECT A.SCREEN AS SCREEN                                  "
              ,"      , COUNT(DISTINCT USER) AS USERS                       "
              ,"      , COST                                                "
              ," FROM INTUIT.SCREEN_USER A                                  "
              ," JOIN INTUIT.SCREEN_COST B                                  "
              ," ON   A.SCREEN = B.SCREEN                                   "
...
screen <- dat$SCREEN
users  <- as.numeric(dat$USERS)
costs  <- as.numeric(dat$COST)
